"""Uncertainty quantification metrics for Bayes by Bayesian method."""
...
def uncertainty_quantification(
    ...
    ensemble_means = df.iloc[:, 2].to_numpy()
    ensemble_stds = df.iloc[:, 3].pow(1/2).to_numpy()
    ensemble_vars = df.iloc[:, 3].to_numpy()
    ...
    calib_ensemble_means = df_calib.iloc[:, 2].to_numpy()
    calib_ensemble_stds = df_calib.iloc[:, 3].pow(1/2).to_numpy()
...
for item in ["LP-Test", "BDB2020+", "EGFR", "M$^{pro}$", "Peptide-Holdout"]:
    uq = uncertainty_quantification(f"ffnn_ecif_{item}_bayes_by_bayesian.csv", f"ffnn_ecif_LP-Val.csv")
    uq_dict[item] = uq

pd.DataFrame(uq_dict).round(3).to_csv("uq_metrics_bayes_by_bayesian.csv", index=True)