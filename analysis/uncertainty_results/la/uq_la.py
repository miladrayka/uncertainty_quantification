"""Uncertainty quantification metrics for Laplace Approximation method."""
...
for item in ["LP-Test", "BDB2020+", "EGFR", "M$^{pro}$", "Peptide-Holdout"]:
    uq = uncertainty_quantification(f"ffnn_ecif_la_{item}.csv", f"ffnn_ecif_la_LP-Val.csv")
    uq_dict[item] = uq

pd.DataFrame(uq_dict).round(3).to_csv("uq_metrics_la.csv", index=True)
